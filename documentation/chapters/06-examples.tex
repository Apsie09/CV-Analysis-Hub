...
Логът от \cref{code:analysis-demo-log} не е пълен системен журнал, а съкратено представяне на съществените състояния, които се наблюдават при демонстрация. Те показват преминаването от качено видео към чакаща задача, последваща обработка, завършен анализ и записани резултати. Това е важно, защото доказва, че приложението не показва статични примерни данни, а изпълнява реален процес на обработка.

\section{Екрани за демонстрация}

Екраните в тази секция показват завършения сценарий в двата потребителски интерфейса и демонстрират, че резултатите от анализа се четат от общия приложен слой. Blazor изгледът представя най-пълната визуализация на анализа, докато Avalonia изгледът доказва, че същите данни и същите ViewModel-и могат да се използват и в настолен клиент.
...
Подобен ефект се наблюдава и при преизползваемата визуализация на списъци от обекти. Потребителят може да променя видимите колони, да избира конкретен ред и да работи с целия съответстващ обект, независимо каква част от него е визуализирана в таблицата. Тази функционалност е приложена както за \texttt{Videos}, така и за \texttt{AnalysisRuns}, което я прави подходящо доказателство за допълнителните точки в проекта.
